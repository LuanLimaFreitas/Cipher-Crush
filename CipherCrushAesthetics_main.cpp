...
int size, ciphermax;
int score_turn;
int print_type = 1;    // 0 = o mais rudimentar,  1 = o com indicacoes dos eixos, 2 = fancyscreen 
...
int main (void)
{
	...
	do
	{
		turn++;
		cout << "#######################\n";
		cout << "turno:" << turn << " pontuacao:" << score << "\n\n";
		
		// Recebe jogadas e verifica sua validade ate receber uma jogoda valida.
		input_try_again=0;
		do
		{
			if (input_try_again==1)
				cout << "esta jogada nao pontua, tente novamente.\n\n";
			screen0.print();
			...
			cin >> ay;
			cin.ignore();
			dir_char='%'; // espero que ninguem digite %
			do
			{
				if(dir_char!='%')
					cout << "direcao invalida, tente n, s, e ou o.\n";
				cout << "escolha dir:";
				cin >> dir_char;
				cin.ignore();
			} while (dir_char!='n' && dir_char!='s' && dir_char!='e' && dir_char!='o');
			cout << "\n";
			...
			if (bx<0 || bx>=size || by<0 || by>=size)
				cout << "movimento fora dos limites do tabuleiro, tente novamente.\n\n";
			else
			{
				cout << "trocando (" << ax << "," << ay << ") com (" << bx << "," << by << "):\n\n";
				screen1.print();
				input_try_again=1;

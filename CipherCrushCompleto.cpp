...
int main (void)
{
	...
	do
	{
		...
		do
		{
			...
			screen1.print();
			getchar();
			screen1.deep_search(screen1.casa[ax][ay],ax,ay,(dir+2)%4);
			buffer=screen1.score_in_search_all;
			screen1.deep_search(screen1.casa[bx][by],bx,by,dir);
			
		} while( buffer<2 && screen1.score_in_search_all<2 );
